<template>
    ...
    <div class="header-page flex justify-between items-center border-b-2 border-gray-200 py-2">
      <div class="title-page flex items-center">
          <i class="fa-solid fa-calendar-check text-4xl mr-3 text-white"></i>
        <div class="title">
          <p class=" text-3xl font-bold text-white">Doctor Schedule</p>
          <p class=" text-sm font-bold text-white">Doctor Schedule Features</p>
        </div>
      </div>
      ...
      <div class="content-page">
        ...
        <div class="table-data mt-5 p-3 bg-white rounded-lg shadow-lg ">
          <div class="header-table flex justify-between items-center border-b-2 border-dashed border-gray-300">
            <p class=" text-3xl font-medium py-2 ">Schedule List</p>
        ...
        <div class="button">
          <button class=" ml-2 p-3 text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 rounded-lg border-2"> Sort<i class="fa-solid fa-arrow-up-short-wide"></i></button>
            ...
            <tbody>
              <tr v-for="(doctors, index) in doctors" :key="index" class="hover:bg-gray-50">
                <td class="py-4 px-4 border-b text-start">{{ doctors.id }}</td>
                <td class="py-4 px-4 border-b text-center">{{ doctors.name }}</td>
                <td class="py-4 px-4 border-b text-center">{{ doctors.weeks }}</td>
                <td class="py-4 px-4 border-b text-center">{{ doctors.time }}</td>
                <td class="py-4 px-4 border-b text-center">
                  <span class="bg-green-200 text-green-600 px-2 py-1 rounded-full">Active</span>
                </td>
                <td class="py-4 px-4 border-b text-center">
                    <router-link to="/schedule-infor">
  ...
  export default {
    ...
    data() {
      return {
        ...
        doctors: [
          { id: 1, name: 'Dr Demo Test', weeks: 'Monday', time: '10:00:00 - 18:00:00' },
          { id: 1, name: 'Dr Demo Test', weeks: 'Monday', time: '10:00:00 - 18:00:00' },
          { id: 1, name: 'Dr Demo Test', weeks: 'Monday', time: '10:00:00 - 18:00:00' },
          { id: 1, name: 'Dr Demo Test', weeks: 'Monday', time: '10:00:00 - 18:00:00' },
    ...
    computed: {
      filteredDepartments() {
        ...
        return this.doctors.slice(start, end).filter(doctors =>
          doctors.name.toLowerCase().includes(this.search.toLowerCase())
        );
      },
    },
    methods: {
