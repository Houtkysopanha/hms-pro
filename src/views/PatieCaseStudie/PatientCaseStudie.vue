<template>
    ...
    <div class="header-page flex justify-between items-center border-b-2 border-gray-200 py-2">
      <div class="title-page flex items-center">
          <i class="fa-solid fa-user-injured text-4xl mr-3 text-white"></i>
        <div class="title">
          <p class=" text-3xl font-bold text-white">Patient Case Studie</p>
          <p class=" text-sm font-bold text-white">Patient Case Studie Features</p>
        </div>
      </div>
      ...
      <div class="content-page">
        <div class="button-add flex justify-start items-center mt-5">
          <router-link to="/AddPatientCaseStudie">
        ...
        <div class="table-data mt-5 p-3 bg-white rounded-lg shadow-lg ">
          <div class="header-table flex justify-between items-center border-b-2 border-dashed border-gray-300">
            <p class=" text-3xl font-medium py-2 ">Patient Case Studie List</p>
          ...
          <button class=" ml-2 p-3 text-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-300 rounded-lg border-2"> Sort<i class="fa-solid fa-arrow-up-short-wide"></i></button>
            ...
            <tbody>
              <tr v-for="(department, index) in departments" :key="index" class="hover:bg-gray-50">
                <td class="py-4 px-4 border-b text-start">{{ department.name }}</td>
                <td class="py-4 px-4 border-b text-start">{{ department.email }}</td>
                <td class="py-4 px-4 border-b text-start">{{ department.phone }}</td>
                <td class="py-4 px-4 border-b text-start">{{ department.foodAllergy }}</td>
                <td class="py-4 px-4 border-b text-start">{{ department.heartDiseases }}</td>
                <td class="py-4 px-4 border-b text-start">{{ department.diabetic }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      departments: [
        {
            name: "Demo Test", email: "demo@gmail.com", phone: "01700000000", foodAllergy: "No", heartDiseases: "No", diabetic: "No"
        },
        {
            name: "Demo Test", email: "demo@gmail.com", phone: "01700000000", foodAllergy: "No", heartDiseases: "No", diabetic: "No"
        },
